...
Функция ПолучитьДанныеДляОтправкиДокументаЗапросЦенПоставщикам_Обновление(ДокЗапроса, ВнешнийID = "")
	...
	Пока Выборка.Следующий() Цикл
		...
		Пока ВыборкаДетали.Следующий() Цикл
			СтруктураСтроки = Новый Структура("externalId,id,productId,productName,quantity,price,deliveryDate,comment");
			...
			СтруктураСтроки.productName  = Строка(ВыборкаДетали.Номенклатура);
...
Функция ПолучитьДанныеДляОтправкиДокументаЗапросЦенПоставщикам(ДокЗапроса)
	...
	СтруктураДанных = Новый Структура("privateRequest,externalId,title,date,currency,description,deliveryAddress,
									  |deliveryTerms,paymentTerms,items");
	...
	Пока Выборка.Следующий() Цикл
		...
		СтруктураДанных.externalId = Строка(Выборка.Ссылка.УникальныйИдентификатор());
		СтруктураДанных.privateRequest = Не Выборка.ТипЗапроса = Перечисления.TK_ТипыЗапроса.Публичный;
		СтруктураДанных.title = "RFQ # " + Строка(Выборка.Номер) + " от " + Формат(Выборка.Дата, "ДФ=dd.MM.yyyy;");
		...
		Пока ВыборкаДетали.Следующий() Цикл
			СтруктураСтроки = Новый Структура("externalId,productId,productName,quantity,price,deliveryDate,comment");
...
//    ТокенСессии = "";
    ...
    РезультатПроверки = ОбработкаИнтеграции.ПолучитьРезультатДокументаЗапросЦенОтКлиентаПроверка(IdЗаказа, ТокенСессии,
		ДанныеПоОрганизации.ОрганизацияНаСайте); 

	Если РезультатПроверки.Свойство("Id") Тогда
