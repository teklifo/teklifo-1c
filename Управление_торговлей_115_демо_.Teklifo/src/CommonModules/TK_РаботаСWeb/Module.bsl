...
//			ДанныеПоОрганизации.ОрганизацияНаСайте);
		...
		Попытка
			НачатьТранзакцию();
			РезультатЗагрузки = TK_ЗагрузкаИСозданиеДанныхСервер.ЗагрузитьДокументЗапросЦенОтКлиента(Результат,Организация);
			
			
			СинхронизоватьЗапросыЦенОтКлиента(РезультатЗагрузки.ДокЗапроса, РезультатЗагрузки.id, РезультатЗагрузки.number, РезультатЗагрузки.latestVersion,
			РезультатЗагрузки.versionId);
