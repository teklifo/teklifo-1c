...
Функция ПолучитьВнешнююСсылкуНаДокументЗаказаЦенПоставщикам(СсылкаНаЗаказ) Экспорт
	ВнешнийID = ПолучитьВнешнийİDДокумента(СсылкаНаЗаказ);
	
	СсылкаНаДокумент = "";
	Если ЗначениеЗаполнено(ВнешнийID) Тогда
		СсылкаНаДокумент = СтрШаблон("https://teklifo.com/rqf/%1",ВнешнийID);
	КонецЕсли;
...
Функция ПолучитьВнешнийİDДокумента(СсылкаНаЗаказ) Экспорт
	Запрос = Новый Запрос("ВЫБРАТЬ
