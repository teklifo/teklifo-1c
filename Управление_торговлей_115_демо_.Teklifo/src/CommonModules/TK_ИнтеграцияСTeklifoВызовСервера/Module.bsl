...
Функция ОпубликоватьЗапросЦенПоставщикамНаTeklifo(ЗапросЦенПоставщикам) Экспорт
	...
	Если Не РезультатВыгрузкиНоменклатуры Тогда
		ОбщегоНазначения.СообщитьПользователю(СтрШаблон(НСтр(
			"ru = 'Не удалось выгрузить номенклатурные позиции на Teklifo. %1'"), ИнтеграцияСTeklifo.УчетнаяЗапись));
		Возврат Результат;
	КонецЕсли;
...
// 
// Параметры:
//  ИмяКонтакногоЛица - Строка- Имя контакного лица
//  Партнер - СправочникСсылка.Партнеры - Партнер
// 
// Возвращаемое значение:
//  СправочникСсылка.КонтактныеЛицаПартнеров, Неопределено - Контактное лицо партнера
Функция КонтактноеЛицоПартнера(ИмяКонтакногоЛица,Партнер) Экспорт
	
	КЛПартнера = Справочники.КонтактныеЛицаПартнеров.НайтиПоНаименованию(СокрЛП(ИмяКонтакногоЛица),Истина, ,Партнер);
	
	Если ЗначениеЗаполнено(КЛПартнера) Тогда
		Возврат КЛПартнера;
	КонецЕсли;
	
	КЛПартнераОбъект = Справочники.КонтактныеЛицаПартнеров.СоздатьЭлемент();
	КЛПартнераОбъект.Наименование = СокрЛП(ИмяКонтакногоЛица);
	КЛПартнераОбъект.Владелец     = Партнер;
	КЛПартнераОбъект.Записать();
	
	Возврат КЛПартнераОбъект.Ссылка;
...
Процедура ЗагрузитьКоммерческиеПредложенияОтПоставщиков()
	...
	Для Каждого ДанныеКоммерческогоПредложенияКЗаписи Из ДанныеКоммерческихПредложенийКЗаписи Цикл
		...
		Если Результат Тогда
			Реквизиты = TK_РаботаСУведомлениямиTeklifo.НоваяСтруктураРеквизитовКоммерческогоПредложенияОтПоставщика();
			ЗаполнитьЗначенияСвойств(Реквизиты,
				ДанныеКоммерческогоПредложенияКЗаписи.КоммерческоеПредложениеПоставщикаОбъект);
			TK_РаботаСУведомлениямиTeklifo.СоздатьУведомлениеОНовомКоммеческомПредложенииОтПоставщика(Реквизиты);
		КонецЕсли;
...
Функция ЗаполнитьИПровестиКоммерческоеПредложениеПоставщика(ДанныеКоммерческогоПредложенияКЗаписи, ВыборкаКонтрагентов)
	...
	Иначе
		...
		ДанныеЗаполненияПартнераКонтрагента.Вставить("ДанныеОрганизацииПартнера",ДанныеКоммерческогоПредложенияКЗаписи.ДанныеКоммерческогоПредложения.company);
	...
	КонецЕсли;
       ...
       КЛПартнера = КонтактноеЛицоПартнера(ДанныеКоммерческогоПредложения.contactPerson,ДокументОбъект.Партнер);
       ДокументОбъект.КонтактноеЛицо = КЛПартнера;
       ДокументОбъект.ИмяКонтактногоЛица = Строка(КЛПартнера);
       ДокументОбъект.Email              = СокрЛП(ДанныеКоммерческогоПредложения.email);
       ДокументОбъект.НомерТелефона      = СокрЛП(ДанныеКоммерческогоПредложения.phone);
	...
	Для Каждого ДанныеПозиции Из ДанныеКоммерческогоПредложения.items Цикл
		...
		СтрокаТабличнойЧасти.ЦенаВключаетНДС = ДанныеКоммерческогоПредложения.vatIncluded;
