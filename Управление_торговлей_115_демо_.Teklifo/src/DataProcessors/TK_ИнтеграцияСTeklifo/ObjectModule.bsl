...
//  Булево - Инициализировать параметры обмена по организации
Функция ИнициализироватьПараметрыОбменаПоОрганизации() Экспорт
	...
	Если Выборка.Следующий() Тогда
		...
		Пароль = ОбщегоНазначения.ПрочитатьДанныеИзБезопасногоХранилища("TK_Teklifo", Строка(
...
Процедура СохранитьУчетнуюЗаписьИнтеграцииСTeklifo() Экспорт
	...
	Попытка
		...
		УчетнаяЗапись = УчетнаяЗаписьОбъект.Записать();

		УстановитьПривилегированныйРежим(Истина);

		ОбщегоНазначения.ЗаписатьДанныеВБезопасноеХранилище("TK_Teklifo", Пароль, Строка(
			УчетнаяЗапись.УникальныйИдентификатор()));
...
Функция ОпубликоватьЗапросЦенПоставщикамНаTeklifo(ЗапросЦенПоставщикам) Экспорт
	...
	ПараметрыHTTPЗапроса.АдресРесурса = СтрШаблон("/api/rfq/%1", ДанныеОпубликованногоДокумента.Идентификатор);
	...
	ДанныеКПубликации = TK_РаботаСJSONВызовСервера.ПолучитьДанныеИзJSON(HTTPОтвет.ПолучитьТелоКакСтроку());

	СохранитьИдентификаторДокументаНаTeklifo(ЗапросЦенПоставщикам, ДанныеКПубликации);
	СохранитьИдентификаторыСтрокДокументаНаTeklifo(ЗапросЦенПоставщикам, ДанныеКПубликации.items);
...
Функция ДанныеКПубликацииЗапросаЦенПоставщикам(ЗапросЦенПоставщикам)
	...
	Выборка = ДанныеЗапросаЦенПоставщикам(ЗапросЦенПоставщикам);
	...
	Если Выборка.Следующий() Тогда
		...
		ДанныеЗапросаПоставщикам.externalId = Строка(Выборка.Документ.УникальныйИдентификатор());
		ДанныеЗапросаПоставщикам.privateRequest = Выборка.ТипЗапроса = Перечисления.TK_ТипыЗапросовЦен.Закрытый;
		ДанныеЗапросаПоставщикам.title = Выборка.Заголовок;
		ДанныеЗапросаПоставщикам.date.from = Формат(Выборка.НачальнаяДатаСбораПредложений, "ДФ=yyyy-MM-dd;");
		ДанныеЗапросаПоставщикам.date.to = Формат(Выборка.КонечнаяДатаСбораПредложений, "ДФ=yyyy-MM-dd;");
...
Функция ДанныеЗапросаЦенПоставщикам(ЗапросЦенПоставщикам)
	...
	Запрос = Новый Запрос("ВЫБРАТЬ
						  ...
						  |ГДЕ
						  |	TK_ЗапросЦенПоставщикамТовары.Ссылка = &ЗапросЦенПоставщикам
						  |;
						  |
						  |////////////////////////////////////////////////////////////////////////////////
						  |ВЫБРАТЬ
						  ...
						  |	ВТ_Документ.Валюта КАК Валюта,
						  |	ВТ_Документ.НачальнаяДатаСбораПредложений КАК НачальнаяДатаСбораПредложений,
						  |	ВТ_Документ.КонечнаяДатаСбораПредложений КАК КонечнаяДатаСбораПредложений,
						  |	ВТ_Документ.ТекстовоеОписание КАК ТекстовоеОписание,
						  |	ВТ_Документ.АдресДоставки КАК АдресДоставки,
						  |	ВТ_Документ.КомментарийКУсловиямДоставки КАК КомментарийКУсловиямДоставки,
						  |	ВТ_Документ.КомментарийКУсловиямОплаты КАК КомментарийКУсловиямОплаты,
						  |	ВТ_Документ.ТипЗапроса КАК ТипЗапроса,
						  |	ВТ_Документ.Номенклатура КАК Номенклатура,
