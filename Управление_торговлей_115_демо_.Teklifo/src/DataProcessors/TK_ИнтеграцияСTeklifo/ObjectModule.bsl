...
Функция ВыгрузитьНоменклатуруНаTeklifo(МассивНоменклатуры) Экспорт
	...
	ПараметрыHTTPЗапроса = НоваяСтруктураПараметровHTTPЗапроса();
	ПараметрыHTTPЗапроса.Заголовки.Вставить("Content-Type", "application/json");
	ПараметрыHTTPЗапроса.Заголовки.Вставить("Cookie", СтрШаблон("user-company=%1",
		ИдентификаторОрганизацииTeklifo));
	ПараметрыHTTPЗапроса.АдресРесурса = "/api/product";
...
Функция ОпубликоватьЗапросЦенПоставщикамНаTeklifo(ЗапросЦенПоставщикам) Экспорт
	...
	ДанныеОпубликованногоЗапроса = TK_РаботаСJSONВызовСервера.ПолучитьДанныеИзJSON(HTTPОтвет.ПолучитьТелоКакСтроку(),"endDate");
...
Функция ДанныеКПубликацииЗапросаЦенПоставщикам(ЗапросЦенПоставщикам)
	...
	Если ВыборкаДокумент.Следующий() Тогда
		...
		ДанныеЗапросаПоставщикам.phone        = ВыборкаДокумент.НомерТелефона;
		ДанныеЗапросаПоставщикам.email        = ВыборкаДокумент.Email;
		ДанныеЗапросаПоставщикам.contactPerson = ВыборкаДокумент.ИмяКонтактногоЛица;
...
Функция ДанныеЗапросаЦенПоставщикам(ЗапросЦенПоставщикам)
	...
	|	Товары.Ссылка.ВремяКонечнойДатыСбораПредложений,
	...
	|	Товары.Ссылка.НомерТелефона,
	|	Товары.Ссылка.Email,
	|	Товары.Ссылка.ИмяКонтактногоЛица
	|ИЗ
...
//	ПериодСбораПредложений = Новый Структура("from, to", '00010101', '00010101');
	...
	ДанныеЗапросаПоставщикам.Вставить("contactPerson", "");
	ДанныеЗапросаПоставщикам.Вставить("email", "");
	ДанныеЗапросаПоставщикам.Вставить("phone", "");
//	ДанныеЗапросаПоставщикам.Вставить("date", ПериодСбораПредложений);
	ДанныеЗапросаПоставщикам.Вставить("endDate", "");
...
Функция СоздатьОбновитьЗапросЦенОтКлиента(ДанныеЗапросаЦен)
	...
	КЛПартнера = TK_ИнтеграцияСTeklifoВызовСервера.КонтактноеЛицоПартнера(ДанныеЗапросаЦен.contactPerson,
		ПартнерИКонтрагент.Партнер);
	...
	ЗапросЦенОтКлиентаОбъект.ВремяКонечнойДатыСбораПредложений = ДанныеЗапросаЦен.endDate - НачалоДня(ДанныеЗапросаЦен.endDate);
...
Функция ДанныеКПубликацииКоммерческогоПредложенияКлиенту(КоммерческоеПредложениеКлиенту)
	...
	Если ВыборкаДокумент.Следующий() Тогда
		...
		ДанныеПредложенияКлиенту.contactPerson = ВыборкаДокумент.ИмяКонтактногоЛица;
		ДанныеПредложенияКлиенту.email = ВыборкаДокумент.Email;
		ДанныеПредложенияКлиенту.phone = ВыборкаДокумент.НомерТелефона;
		...
		Пока ВыборкаДетали.Следующий() Цикл
			...
			ПозицияЗапросаПоставщикам.vatRate = TK_ИнтеграцияСTeklifoВызовСервера.ЗначениеСтавкиНДСНаTeklifo(
				ВыборкаДетали.СтавкаНДС);
