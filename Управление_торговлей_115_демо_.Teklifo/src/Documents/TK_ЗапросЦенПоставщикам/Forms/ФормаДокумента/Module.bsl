...
Процедура КомандаСкопироватьСсылкуВБуфер(Команда)
	...
	СсылкаДляЗапуска = TK_ОбщегоНазначенияВызовСервера.ПолучитьСсылкуНаДокумент_ДляЗапуска(Объект.Ссылка);
...
Асинх Процедура НачатьПубликацию()
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		...
		Если Результат Тогда
			...
			TK_ОбщегоНазначенияВызовСервера.УстановитьСтатусДокументаЗаказаЦенПоставщику(Объект.Ссылка,
...
Функция РезультатОтправкиДокумента()
	Возврат TK_РаботаСWebВызовСервера.ПолучитьРезультатОтправкиДокументаЗапросЦенПоставщикам(Объект.Ссылка);
КонецФункции

&НаСервере
Процедура ВывестиСсылкуНаДокумент()

	СсылкаНаДокумент = TK_ОбщегоНазначенияВызовСервера.ПолучитьВнешнююСсылкуНаДокументЗаказаЦенПоставщикам(Объект.Ссылка);
...
Процедура ОтобразитьТекущийСтатусДокумета()
	...
	СтатусДокумента = TK_ОбщегоНазначенияВызовСервера.ПолучитьСтатусДокументаЗаказаЦенПоставщику(Объект.Ссылка);

	Если Не ЗначениеЗаполнено(СтатусДокумента) Тогда
		СтатусДокумента = Перечисления.TK_СтатусыПубликацииЗапросовЦен.НеОпубликован;
	КонецЕсли;
